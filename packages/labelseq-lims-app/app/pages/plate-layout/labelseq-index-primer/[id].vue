...
onMounted(async() => {
    ...
    plateLayout.wellContentsDisplayConfig.value = {
        colorBy: ['labelseqIndexPrimer.id'],
        selectionTableRecordIdPaths: ['labelseqIndexPrimer.id'],
        tooltip: (well: any) => {
            const wellCoordinate = `${wellCoordinateToChar(well.y)}${well.x}`
            const labelseqIndexPrimerName = _.get(_.find(well.wellContents, (wc: any) => _.get(wc, 'wellable.labelseqIndexPrimer')), 'wellable.labelseqIndexPrimer.name')
            return labelseqIndexPrimerName ? `${wellCoordinate}: ${labelseqIndexPrimerName}` : wellCoordinate
        },
    }
    loadPlate()
})
