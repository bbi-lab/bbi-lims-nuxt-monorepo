...
onMounted(async() => {
    ...
    plateLayout.wellContentsDisplayConfig.value = {
        colorBy: ['retrieverPrimer.id'],
        selectionTableRecordIdPaths: ['retrieverPrimer.id'],
        symbol: (well: any) => {
            const retrieverPrimerDirection = _.get(_.find(well.wellContents, (wc: any) => _.get(wc, 'wellable.retrieverPrimer')), 'wellable.retrieverPrimer.direction')
            return retrieverPrimerDirection ? retrieverPrimerDirection[0].toUpperCase() : ''
        },
        tooltip: (well: any) => {
            const wellCoordinate = `${wellCoordinateToChar(well.y)}${well.x}`
            const retrieverPrimerName = _.get(_.find(well.wellContents, (wc: any) => _.get(wc, 'wellable.retrieverPrimer')), 'wellable.retrieverPrimer.name')
            return retrieverPrimerName ? `${wellCoordinate}: ${retrieverPrimerName}` : wellCoordinate
        },
    }
    loadPlate()
})
...
const columnDefs: ColumnDefinitions = {
    ...
    name: { index: 3 },
    seq: { index: 4 },
    seqRevComp: { header: 'Rev Comp Seq', index: 5 },
    direction: { index: 6 },
}
...
const whereClause = {
    "==": [
        { "var": "wellable.retrieverPrimer.direction" },
        "reverse"
    ],
}
